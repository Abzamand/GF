'user strict';
angular.module('DeliveryOrderApp').controller(
		'DeliveryOrderCtrl',
		...
		function($scope, $rootScope, Restful, CONSTANTS, $rootScope,$location,$localStorage) {
			...
			$scope.isUser = true;
			$scope.tempStatus = ["CREATE DO COMPLETED","CREATE DO FAILED","ASSIGN DO COMPLETED", "ASSIGN DO FAILED","PGI COMPLETED","PGI FAILED", "CANCELED"]
			$scope.tempDocType = ["PO STO","SO"]
			
			
			/*$scope.isSlocNull = function(item) {
			...
			Restful.httpPost('/api/v1/report/deliveryOrder',$scope.filter, 
			...
			$scope.export = function(data){
				...
				const worksheet = XLSX.utils.json_to_sheet(gson, {
					header:["ID","DO NO","DATE","SLOC","DESTINATION","DOC NO","DOC DATE",
					"DOC TYPE","PGI NO", "POD STATUS", "STATUS", "ITEM NO", "DOC ITEM NO", 
					"MATERIAL NO", "MATERIAL DESC", "ORDER QTY", "ORDER UOM", "DO QTY", "DO UOM", "BATCH NO", "ACTUAL QTY", "ACTUAL UOM"]
					}
				);
				const workbook = {
					Sheets: {
				...
				saveAsExcel(excelBuffer, 'Report Delivery_Order_' + new Date().toJSON().slice(0,10).replace(/-/g,'/'));	
			}
			...
			Restful.httpPost('/api/v1/report/deliveryOrderExport',$scope.filter, 
				 null, callBackGetList, callBackErrorGetList, $scope, false, false);
			}
} ]);