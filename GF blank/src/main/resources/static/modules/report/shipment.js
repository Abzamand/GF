'user strict';
angular.module('ShipmentApp').controller(
		'ShipmentCtrl',
		...
		function($scope, $rootScope, Restful, CONSTANTS, $rootScope,$location,$localStorage) {
			...
			var callBackErrorGetList = function(){$('#loaderContainer').css('display','none');}
				Restful.httpPost('/api/v1/report/Shipment',$scope.filter, 
			...
			$scope.export = function(data){
				...
				const worksheet = XLSX.utils.json_to_sheet(gson, {
					header:["SHIPMENT NO", "SHIPMENT DATE", "SLOC", "PLANT", "STATUS", 
					"DO NO", "DO DATE"]
					}
				);
				const workbook = {
				...
				saveAsExcel(excelBuffer, 'Report Shipment_' + new Date().toJSON().slice(0,10).replace(/-/g,'/'));	
			}
			...
			Restful.httpPost('/api/v1/report/shipmentExport',$scope.filter, 
				 null, callBackGetList, callBackErrorGetList, $scope, false, false);
			}
} ]);