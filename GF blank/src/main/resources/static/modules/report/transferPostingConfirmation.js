'user strict';
angular.module('TransferPostingConfirmationApp').controller(
		'TransferPostingConfirmationCtrl',
		...
		function($scope, $rootScope, Restful, CONSTANTS, $rootScope,$location,$localStorage) {
			...
			Restful.httpPost('/api/v1/report/transferPostingConfirmation',$scope.filter, 
			...
			$scope.export = function(data){
				...
				const worksheet = XLSX.utils.json_to_sheet(gson, {
					header:["TP CONFIRMATION NO", "VENDOR STOCK CONFIRMATION", "DATE", "VENDOR NAME","SLOC","PLANT","TRANSFER POSTING NO","TRANSFER POSTING DATE","STATUS",
					"ITEM NO","DOC ITEM NO", "MATERIAL NO", "MATERIAL DESC", "BATCH NO", "QTY", "UOM", "NOTES"]
					}
				);
				const workbook = {
					Sheets: {
				...
				saveAsExcel(excelBuffer, 'Report Transfer_Posting_Confirmation_' + new Date().toJSON().slice(0,10).replace(/-/g,'/'));	
			}
			...
			Restful.httpPost('/api/v1/report/transferPostingConfirmationExport',$scope.filter, 
				 null, callBackGetList, callBackErrorGetList, $scope, false, false);
			}
} ]);